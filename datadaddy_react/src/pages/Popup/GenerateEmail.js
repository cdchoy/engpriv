...
export default function generateEmail(recipients, sender) {
    ...
    let emailUrl = urlprefix + emailTo + emailSubject + emailBody + sender;
    // todo: READ in sender from chrome.storage
    return emailUrl;
    // return chrome.storage.sync.get('settings').then(data => {
    //     const emailSignature = data.settings.fullname;
    //     return emailUrl + emailSignature;
    // });
}

function DataBrokerEmailForm(props) {
    return (
        <div>
            <div>
